        ...
        <div class="p-6">
          <form @submit.prevent="submitForm">
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <!-- Nilai Arisan -->
              <div>
                <label for="amount" class="block text-sm font-medium text-gray-700 mb-1">Nilai Arisan Per Orang (ETH)</label>
                <input 
                  type="number" 
                  id="amount" 
                  v-model="form.amount"
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                  placeholder="Contoh: 0.1 ETH"
                  required
                />
              </div>
...
const submitForm = async () => {
  try {
    ...
    // 🔹 STEP 1: Simpan grup ke backend
    const res = await api.post('/arisanGroup', form.value, {
      headers: {
    ...
    try {
      // 🔹 STEP 2: Deploy kontrak
      const contractAddress = await deployArisanContract()

      // 🔹 STEP 3: Simpan contract_address ke backend
      ...
      console.error('❌ Gagal deploy kontrak:', err)
      ...
      notificationMessage.value = 'Gagal deploy kontrak. Data grup dibatalkan.'
      notificationType.value = 'error'
      showNotification.value = true
    }
